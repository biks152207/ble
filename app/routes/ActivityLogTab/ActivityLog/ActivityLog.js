...
class ActivityLog extends Component {
  ...
  calculateTotalTymebByStatus(status,format){
    ...
    let calcTime = '00:00'
    let totalMinutes = 0;
    if(this.props.activityLog.activityDayLog && this.props.activityLog.activityDayLog.logs){

      this.props.activityLog.activityDayLog.logs.forEach((log,index)=>{
        if(log.status == status){

          let difference = moment(log.timeEnd || endDateTime).diff(log.timeStart, "minutes");
          totalMinutes += difference;
          if(index==self.props.activityLog.activityDayLog.logs.length-1){
  ...
  render() {
    ...
    let dataForGraph = [];
    if (!this.state.loading && this.props.activityLog.activityDayLog && this.props.activityLog.activityDayLog.logs) {
      this.props.activityLog.activityDayLog.logs.forEach((log) =>{
        let data = {}
        let time;
        let status;
        if (log.status === 'ON') {
    ...
    return(
      <View style={s.Container}>
        <View style={[{backgroundColor:common.blackColor, height: 55 , flexDirection:'row'}]}>
          <TouchableHighlight onPress={() => {navigate('ActivityStatus')}} underlayColor="transparent" style={[{flex:0.1,width:60,alignItems:'center',justifyContent:'center'}]}>
            <Image
        ...
        <View style={{height : (window.height)-55}}>
          <ScrollView>
            ...
            {!this.state.loading && this.props.activityLog.activityDayLog && this.props.activityLog.activityDayLog.webLogs && this.props.activityLog.activityDayLog.webLogs.length>0 && <View style={{backgroundColor:common.whiteColor}}>
                <TouchableHighlight onPress={() => {navigate('EditedActivityLog',{day:state.params.day, nextDayLog:state.params.nextLog})}} underlayColor={common.tuchableUnderlayGreenColor} style={[{width:100,height:30,backgroundColor:common.greenColor,flexDirection:'row',alignItems:'center',justifyContent:'center'}]}>
                  <View><Text style={{color:common.whiteColor,fontSize:18}}>Edited Log</Text></View>
                </TouchableHighlight>
              </View>
            }
            {!this.state.loading && this.props.activityLog.activityDayLog && this.props.activityLog.activityDayLog.logs && this.props.activityLog.activityDayLog.logs.map((logData, index)=>{
              return (
                <View>

                  {true && <TouchableHighlight onPress={() => {navigate('AddEvent',{day:state.params.day, log:logData, dataForGraph})}} underlayColor={common.tuchableUnderlayGreenColor} style={[{}]}>
          ...
          <View style={{backgroundColor:common.whiteColor,alignItems:'center'}}>
            <TouchableHighlight onPress={() => {navigate('AddEvent',{day:state.params.day, dataForGraph})}} underlayColor={common.tuchableUnderlayGreenColor} style={[{marginTop:30,width:window.width-50,marginBottom:50}]}>
